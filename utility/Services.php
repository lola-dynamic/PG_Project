...
class Services
{
    ...
    public function sendEmailToRemovedStudent($email, $message)
    {
        $subject = "Your seminar has been cancelled ( csepostgraduateseminar@gmail.com )";
        $this->sendEmails($email, $subject, $message);
    }

    public function sendPgSeminarNotice2Supervisor($email, $message)
    {
        $subject = "PG Seminar Notice ( csepostgraduateseminar@gmail.com )";
        $this->sendEmails($email, $subject, $message);
    }
...
//        $mail->SMTPDebug = 3;
        ...
        $mail->Password = "dinner2158";
        $mail->SMTPSecure = "ssl";
        $mail->Port = 465;

        $mail->From = 'info@realcodestuffs.com';
        $mail->FromName = "PG coordinator";
...
//            }
            $final_semester = (int)$calculated_semester + (int)$leave_of_absence;
...
//            Rain is the second semester denoted with R
        ...
        elseif ($admin_semester == 'H' && $student_semester == 'R') {
            $new_semester = (floatval($year) * 2) + 1;
        }
        elseif ($admin_semester == 'R' && $student_semester == 'H') {
            $new_semester = (floatval($year) * 2) + 1;
        }
    ...
    function calculateYear($student_session) {
        ...
        $year = (int)$admin_session - (int)$student_session;
